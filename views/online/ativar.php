    ...
    foreach ($servidores as $servidor) {
        ...
        if (ssh2_auth_password($connection, $servidor["usuario"], $servidor["senha"])) {
            ...
            if (trim($response) === "active") {
                $_SESSION["configerr"] = "Erro ao ativar online <br> O serviço já está ativo.";
                header("Location: " . $_SERVER["HTTP_REFERER"]);
                exit;
            }
            ssh2_exec($connection, "sudo systemctl enable online.service >/dev/null 2>&1 &");
            ssh2_exec($connection, "sudo systemctl start online.service >/dev/null 2>&1 &");
...
if (1 < count($servidores_erro)) {
    $msg_sucesso = "<div>Online ativado com sucesso nos seguintes servidores:</div>";
    $msg_erro = "<div>Erro ao ativado online nos seguintes servidores:</div>";
    ...
    if (count($servidores_erro) === 1) {
        $_SESSION["configerr"] = "Erro ao ativado online no servidor: " . $servidores_erro[0];
    } else {
        $_SESSION["config"] = "<div>Online ativado com sucesso!</div>";
    }
}
header("Location: " . $_SERVER["HTTP_REFERER"]);
exit;
